...
const autoRoleConfigs = pgTable('auto_role_configs', {
  id: serial('id').primaryKey(),
  guildId: text('guild_id').notNull().references(() => guilds.id, { onDelete: 'cascade' }),
...
const commandStats = pgTable('command_stats', {
  id: serial('id').primaryKey(),
  guildId: text('guild_id').references(() => guilds.id, { onDelete: 'cascade' }),
...
const welcomeConfigsRelations = relations(welcomeConfigs, ({ one }) => ({
  guild: one(guilds, {
...
const userWarningsRelations = relations(userWarnings, ({ one }) => ({
  guild: one(guilds, {
...
const autoRoleConfigsRelations = relations(autoRoleConfigs, ({ one }) => ({
  guild: one(guilds, {
...
const commandStatsRelations = relations(commandStats, ({ one }) => ({
  guild: one(guilds, {
...
module.exports = {
  ...
  commandStats,
  guildsRelations,
  welcomeConfigsRelations,
  embedTemplatesRelations,
  moderationLogsRelations,
  userWarningsRelations,
  autoRoleConfigsRelations,
  commandStatsRelations
};